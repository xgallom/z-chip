...
const Self = @This();
const InstTable = [@sizeOf(u16)]OpCode;
...
pub fn decode(i: u16) Self {
    return .init(switch (in(i, 0)) {
        ...
        0x3 => .se_rc,
        0x4 => .sne_rc,
        0x5 => if (in(i, 3) == 0x0) .se_rr else .invalid,
        0x6 => .ld_rc,
        0x7 => .add_rc,
        0x8 => switch (in(i, 3)) {
            0x0 => .ld_rr,
...
inline fn inst(data: [byte_size]u8) u16 {
...
test inst {
    try std.testing.expectEqual(inst(.{ 0xFF, 0xAA }), 0xFFAA);
}
